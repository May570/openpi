...
def _parse_index_spec(spec: Optional[str], dim: int) -> Optional[List[int]]:
    """
    Parse an index spec like:
      "0,1,2,5" or "0-6,10-13" or "".
    Returns sorted unique indices within [0, dim-1].
    """
    if spec is None:
        return None
    spec = str(spec).strip()
    ...
    for part in spec.split(","):
        ...
        if "-" in part:
            ...
            out.append(int(part))
    out = sorted({i for i in out if 0 <= i < dim})
    return out if out else None


def _default_arm_and_gripper_indices(state_dim: int) -> Tuple[List[int], List[int]]:
    """
    Best-effort defaults for common dual-arm setups:
    - If state_dim is a multiple of 7 and >= 14, treat every 7 dims as one arm (last dim is gripper).
      e.g. ALOHA: 14 dims -> grippers at 6 and 13.
    - Else if state_dim >= 2, treat last 2 dims as grippers, the rest as arm joints.
    """
    if state_dim >= 14 and state_dim % 7 == 0:
        gripper = [i for i in range(6, state_dim, 7)]
        arm = [i for i in range(state_dim) if i not in gripper]
        return arm, gripper
    if state_dim >= 2:
        gripper = [state_dim - 2, state_dim - 1]
        arm = list(range(0, state_dim - 2))
        return arm, gripper
    return list(range(state_dim)), []


def _state_l2_delta(prev_state: List[float], curr_state: List[float], idx: List[int]) -> float:
    if not idx:
        return 0.0
    a = np.asarray([prev_state[i] for i in idx], dtype=np.float32)
    b = np.asarray([curr_state[i] for i in idx], dtype=np.float32)
    return float(np.linalg.norm(b - a, ord=2))


def _image_mean_abs_diff(prev_img: Image.Image, curr_img: Image.Image, size: int = 64) -> float:
    """
    Mean absolute pixel difference on downsampled grayscale images.
    Returns a value in [0, 255].
    """
    p = prev_img.convert("L").resize((size, size), Image.BILINEAR)
    c = curr_img.convert("L").resize((size, size), Image.BILINEAR)
...
class EpisodeFramewiseTester:
    def __init__(
        ...
        arm_static_thresh: float = 1e-3,
        img_static_thresh: float = 1.0,
        gripper_motion_thresh: float = 5e-3,
        arm_gripper_only_max: float = 2e-3,
        state_arm_indices: Optional[str] = None,
        state_gripper_indices: Optional[str] = None,
        img_diff_downsample: int = 64,
    ):
        ...
        self.arm_static_thresh = float(arm_static_thresh)
        self.img_static_thresh = float(img_static_thresh)
        self.gripper_motion_thresh = float(gripper_motion_thresh)
        self.arm_gripper_only_max = float(arm_gripper_only_max)
        self.state_arm_indices_spec = state_arm_indices
        self.state_gripper_indices_spec = state_gripper_indices
        self.img_diff_downsample = int(img_diff_downsample)
    ...
    def run(self) -> bool:
        ...
        for i in range(self.T):
            # From the 2nd frame on, optionally skip inference for static frames.
            skipped = False
            static_dbg: Dict[str, float] = {}
            if i >= 1:
                is_static, static_dbg = self._is_static_frame(i)
                if is_static:
                    skipped = True
            ...
            if skipped:
                ok = True
                latency = 0.0
                infer_json = {"success": True, "skipped": True, "static_metrics": static_dbg}
            else:
            ...
            # 误差计算
            if infer_json.get("skipped", False):
                # static frame: set mse=0 (and rmse=0) by requirement
                true_seq = np.zeros((0, 0), dtype=np.float32)
                pred_seq = np.zeros((0, 0), dtype=np.float32)
            else:
            ...
            if i % 50 == 0 or i == self.T - 1:
                print(f"[{i+1}/{self.T}] ok={ok} skipped={infer_json.get('skipped', False)} L={L} mse={self.frame_errors_mse[-1]:.6f} latency={latency:.3f}s")
    ...
    def _get_state_list(self, i: int) -> List[float]:
        row = self.df.iloc[i]
        return safe_to_list(row.get(self.col_state, []))

    def _is_static_frame(self, i: int) -> Tuple[bool, Dict[str, float]]:
        """
        Decide whether frame i is 'static' relative to frame i-1.
        Returns (is_static, debug_metrics).
        Logic:
          1) If arm (excluding grippers) changes < arm_static_thresh -> static (no inference).
          2) Else, check image diff:
             - If image diff >= img_static_thresh -> NOT static.
             - If image diff < img_static_thresh:
                 * If the change is mainly gripper open/close (gripper_delta >= gripper_motion_thresh
                   and arm_delta < arm_gripper_only_max) -> NOT static.
                 * Otherwise treat as jitter/near-static -> static.
        """
        prev_state = self._get_state_list(i - 1)
        curr_state = self._get_state_list(i)

        dim = min(len(prev_state), len(curr_state))
        prev_state = prev_state[:dim]
        curr_state = curr_state[:dim]

        arm_idx = _parse_index_spec(self.state_arm_indices_spec, dim)
        grip_idx = _parse_index_spec(self.state_gripper_indices_spec, dim)
        if arm_idx is None or grip_idx is None:
            arm_default, grip_default = _default_arm_and_gripper_indices(dim)
            if arm_idx is None:
                arm_idx = arm_default
            if grip_idx is None:
                grip_idx = grip_default

        arm_delta = _state_l2_delta(prev_state, curr_state, arm_idx)
        grip_delta = _state_l2_delta(prev_state, curr_state, grip_idx)

        dbg: Dict[str, float] = {
            "arm_delta_l2": arm_delta,
            "gripper_delta_l2": grip_delta,
            "img_diff_meanabs": float("nan"),
        }

        if arm_delta < self.arm_static_thresh:
            return True, dbg

        prev_img = self._get_cam_high_pil(i - 1)
        curr_img = self._get_cam_high_pil(i)
        if prev_img is None or curr_img is None:
            return False, dbg

        img_diff = _image_mean_abs_diff(prev_img, curr_img, size=self.img_diff_downsample)
        dbg["img_diff_meanabs"] = img_diff

        if img_diff >= self.img_static_thresh:
            return False, dbg

        if (grip_delta >= self.gripper_motion_thresh) and (arm_delta < self.arm_gripper_only_max):
            return False, dbg
...
def main():
    ...
    ap.add_argument("--out-dir", default="./out_eval7")
    ...
    # --- static frame skipping ---
    ap.add_argument("--arm-static-thresh", type=float, default=1e-3, help="手臂(不含夹爪)状态L2变化阈值，小于则视为静止并跳过推理")
    ap.add_argument("--img-static-thresh", type=float, default=1.0, help="cam_high画面变化阈值(灰度下采样均值绝对差, 0-255)，小于则认为画面基本不变")
    ap.add_argument("--gripper-motion-thresh", type=float, default=5e-3, help="夹爪状态L2变化阈值，大于则认为夹爪确实在动")
    ap.add_argument("--arm-gripper-only-max", type=float, default=2e-3, help="判定为'仅夹爪开合'时，手臂(不含夹爪)变化需小于该阈值")
    ap.add_argument("--state-arm-indices", type=str, default=None, help="手臂维度索引(不含夹爪)，例如 '0-5,7-12'；为空则自动推断")
    ap.add_argument("--state-gripper-indices", type=str, default=None, help="夹爪维度索引，例如 '6,13'；为空则自动推断")
    ap.add_argument("--img-diff-downsample", type=int, default=64, help="画面差异计算的下采样尺寸(默认64)")
    args = ap.parse_args()
    ...
    tester = EpisodeFramewiseTester(
        ...
        arm_static_thresh=args.arm_static_thresh,
        img_static_thresh=args.img_static_thresh,
        gripper_motion_thresh=args.gripper_motion_thresh,
        arm_gripper_only_max=args.arm_gripper_only_max,
        state_arm_indices=args.state_arm_indices,
        state_gripper_indices=args.state_gripper_indices,
        img_diff_downsample=args.img_diff_downsample,
    )
    ok = tester.run()
