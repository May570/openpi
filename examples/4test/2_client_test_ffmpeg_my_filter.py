...
class EpisodeFramewiseTester:
    ...
    def run(self) -> bool:
        ...
        # ============================
        # 跳过推理的帧（0-based index）
        # ============================
        skip_frames = set()

        # adjust_bottle 001
        # skip_frames.update([1, 2])
        # skip_frames.update(range(39, 45))     # 39–44
        # skip_frames.update(range(56, 61))     # 56–60
        # skip_frames.update(range(69, 82))     # 69–81
        # skip_frames.update(range(95, 99))     # 95–98
        # skip_frames.update(range(134, 141))   # 134–140

        # blocks_ranking_rgb 000
        skip_frames.update(range(31, 35))
        skip_frames.update(range(45, 52))
        skip_frames.update(range(83, 86))
        skip_frames.update(range(104, 107))
        skip_frames.update(range(118, 127))
        skip_frames.update([141, 142])
        skip_frames.update(range(153, 156))
        skip_frames.update(range(192, 195))
        skip_frames.update(range(206, 210))
        skip_frames.update(range(222, 231))
        skip_frames.update(range(242, 245))
        skip_frames.update(range(260, 263))
        skip_frames.update(range(274, 282))
        skip_frames.update(range(309, 311))
        skip_frames.update(range(329, 332))
        skip_frames.update(range(344, 348))
        skip_frames.update(range(363, 368))
        skip_frames.update(range(379, 381))
        skip_frames.update(range(401, 404))
        skip_frames.update(range(415, 420))
        ...
        for i in range(self.T):
            ...
            if i in skip_frames:
                ok = False
                infer_json = {"skipped": True}
                latency = None

                self.http_ok.append(False)
                self.latencies.append(None)

                self.frame_valid_steps.append(0)
                self.frame_errors_mse.append(np.nan)
                self.frame_errors_rmse.append(np.nan)

                # 仍然保存 per-frame json，方便你回溯
                frame_dump = {
                    "frame_idx": i,
                    "ok": False,
                    "skipped": True,
                ...
                with open(
                    os.path.join(per_frame_dir, f"infer_{i:06d}.json"),
                    "w",
                    encoding="utf-8",
                ) as f:
                    json.dump(frame_dump, f, ensure_ascii=False, indent=2)

                # 视频照常写（如果你希望跳过帧也出现在视频里）
...
def main():
    ...
    ap.add_argument("--out-dir", default="./out_eval6")
