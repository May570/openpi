...
# 加载模型
MODEL_PATH = "/share/project/wujiling/openpi/checkpoints/pi05_grab_roller/pi05_grab_roller/9999"
...
def load_policy():
    ...
    try:
        ...
        # 使用openpi的配置和策略
        config = _config.get_config("pi05_grab_roller")
...
def replay_api():
    ...
    try: 
        ...
        qpos = np.load('/home/admin123/Desktop/actions_episode_000003.npy').tolist()
...
def infer_api():
    ...
    try:
        ...
        state = data.get('state')  # 如果是 test 就改成 state，否则用 qpos
        # print(state)
        images_tensor = process_images(images) # 处理图片数据
        # print(images_tensor)
        ...
        # obs = {
        #     "observation/image": images_tensor['image'],
        #     "observation/wrist_image": images_tensor['wrist_image'],
        #     "observation/state": np.array(state[0]).astype(np.float32),
        #     "prompt": data.get('prompt'),
        # }
