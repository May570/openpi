...
# 加载模型
MODEL_PATH = "/share/project/wujiling/models/finetune/pi05_pnp_orange/20k"
...
def infer_api():
    ...
    try:
        ...
        state = data.get('qpos')  # 和真机连用 qpos，和测试脚本连用 state
        ...
        # # === 保存到 infer_actions/chunk_xxx.npy ===
...
def _parse_obs_from_json(data):
    ...
    images_tensor = process_images(images)  # 解码逻辑：base64->PIL->Tensor
    obs = {
        "images": {
