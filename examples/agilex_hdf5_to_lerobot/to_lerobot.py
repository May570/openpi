...
class DatasetConfig:
    ...
    # ===== 新增：episode 体检阈值与行为 =====
    enable_episode_sanity_check: bool = True
    reject_nan_inf: bool = True
    # 以“全维均值向量的按元素绝对值最大值（∞范数）”做阈值；std 用各维最大值
    state_abs_mean_max: float = 20.0
    state_std_max: float = 20.0
    action_abs_mean_max: float = 20.0
    action_std_max: float = 20.0
    # 少帧或全零等情况直接丢弃
    min_frames_after_filter: int = 10
    # 日志：把被丢弃的 episode 记到一个 csv，方便追踪
    reject_log_path: str = "rejected_episodes.csv"
...
def _episode_stats_ok(state_np: np.ndarray,
                      action_np: np.ndarray,
                      cfg: DatasetConfig) -> tuple[bool, dict]:
    """
    返回 (是否通过, 统计字典)
    统计字典里包括各项最大绝对均值、最大std、是否含 NaN/Inf、帧数等
    """
    stats = {
        "frames": int(state_np.shape[0]),
        "state_abs_mean_max": float(np.max(np.abs(np.nanmean(state_np, axis=0)))) if state_np.size else np.inf,
        "state_std_max": float(np.max(np.nanstd(state_np, axis=0))) if state_np.size else np.inf,
        "action_abs_mean_max": float(np.max(np.abs(np.nanmean(action_np, axis=0)))) if action_np.size else np.inf,
        "action_std_max": float(np.max(np.nanstd(action_np, axis=0))) if action_np.size else np.inf,
        "has_nan_inf": bool(
            np.any(~np.isfinite(state_np)) or np.any(~np.isfinite(action_np))
        ),
    }

    if stats["frames"] < cfg.min_frames_after_filter:
        return False, stats
    if cfg.reject_nan_inf and stats["has_nan_inf"]:
        return False, stats
    if stats["state_abs_mean_max"] > cfg.state_abs_mean_max:
        return False, stats
    if stats["state_std_max"] > cfg.state_std_max:
        return False, stats
    if stats["action_abs_mean_max"] > cfg.action_abs_mean_max:
        return False, stats
    if stats["action_std_max"] > cfg.action_std_max:
        return False, stats

    return True, stats


def _append_reject_log(csv_path: str, ep_path: str, task: str, stats: dict):
    header = [
        "episode_path", "task", "frames",
        "state_abs_mean_max", "state_std_max",
        "action_abs_mean_max", "action_std_max",
        "has_nan_inf"
    ]
    row = [
        ep_path, task, stats.get("frames", ""),
        stats.get("state_abs_mean_max", ""),
        stats.get("state_std_max", ""),
        stats.get("action_abs_mean_max", ""),
        stats.get("action_std_max", ""),
        stats.get("has_nan_inf", ""),
    ]
    try:
        ...
        print(f"[WARN] failed to write reject log {csv_path}: {e}")
...
def populate_dataset2(
    ...
    for ep_idx in tqdm.tqdm(episodes):
        ...
        # 3) ===== 新增：episode 体检（均值/方差、NaN、帧数等）=====
        if dataset_config.enable_episode_sanity_check:
            ok, stats = _episode_stats_ok(state, action, dataset_config)
            if not ok:
                print(f"[SKIP] {ep_path.name} (task={task}) failed sanity check: {stats}")
                _append_reject_log(dataset_config.reject_log_path, str(ep_path), task, stats)
                continue  # 直接跳过这个 episode
...
def port_task_with_subtasks(
    task_dir: Path,               # 例如: Path("/share/6task/10task/fold_clothes")
    repo_id: str,                 # 例如: "my_6task_10task"
    *,
    episodes: list[int] | None = None,
    ...
    mode: Literal["video", "image"] = "image",
    dataset_config: DatasetConfig = DEFAULT_DATASET_CONFIG,
    sub_prompt_map: dict[str, str] | None = None,   # 形如 {"white": "...", "black": "..."}
):
    """
    将一个任务(如 fold_clothes)下的多个子任务(white/black/...) 合并到同一数据集:
      - 数据集ID: f"{repo_id}/{task_name}"
      - 子任务通过 frame['task'] = 对应 prompt 区分
    子任务目录结构示例:
      task_dir/
        white/
          2024-10-01/episode_*.hdf5
          2024-10-02/episode_*.hdf5
        black/
          2024-10-03/episode_*.hdf5
    """
    task_name = task_dir.name
    repo_task_id = f"{repo_id}/{task_name}"

    # 枚举子任务目录(直接子目录)
    subtask_dirs = [d for d in task_dir.iterdir() if d.is_dir()]
    if not subtask_dirs:
        subtask_dirs = [task_dir]  # 没有子任务就退化为原逻辑

    # 收集全部episode文件用于推断 has_velocity / has_effort，并创建空数据集(一次)
    all_h5 = []
    for sd in subtask_dirs:
        all_h5.extend(sorted(sd.rglob("episode_*.hdf5")))
    if not all_h5:
        raise ValueError(f"No HDF5 files found under {task_dir}")

    if (LEROBOT_HOME / repo_task_id).exists():
        shutil.rmtree(LEROBOT_HOME / repo_task_id)

    dataset = create_empty_dataset(
        repo_task_id,
        robot_type="mobile_aloha" if is_mobile else "aloha",
        mode=mode,
        has_effort=has_effort(all_h5),
        has_velocity=has_velocity(all_h5),
        dataset_config=dataset_config,
    )

    # 逐个子任务写入，给每个子任务设置不同的 prompt
    for sd in subtask_dirs:
        sub_name = sd.name
        hdf5_files = sorted(sd.rglob("episode_*.hdf5"))
        if not hdf5_files:
            print(f"  - Skip empty subtask: {sub_name}")
            continue

        # 选择 prompt：优先用 sub_prompt_map，其次默认 "task/subtask"
        prompt = (sub_prompt_map or {}).get(sub_name, f"{task_name}/{sub_name}")
        print(f"  ▶ Subtask {sub_name}: {len(hdf5_files)} episodes, prompt='{prompt}'")

        dataset = populate_dataset2(
            dataset,
            hdf5_files,
            task=prompt,                 # 关键：同一任务中，不同子任务写不同 prompt
            episodes=episodes,
            dataset_config=dataset_config,
        )
...
def port_task_with_twolevel_subtasks(
    task_dir: Path,                # e.g. /share/6task/10task/open-close
    repo_id: str,
    *,
    episodes: list[int] | None = None,
    push_to_hub: bool = False,
    ...
    mode: Literal["video", "image"] = "image",
    dataset_config: DatasetConfig = DEFAULT_DATASET_CONFIG,
    prompt_map: dict[str, str] | None = None,   # 形如 {"黑色桌布/开": "...", "卡其色桌布/关": "..."}
):
    """
    支持两层子任务: 例如 open-close/颜色/动作/episode_*.hdf5
    每个 episode 的 prompt 由颜色+动作组合决定。
    """
    task_name = task_dir.name
    repo_task_id = f"{repo_id}/{task_name}"

    # 第一级子任务目录（颜色）
    color_dirs = [d for d in task_dir.iterdir() if d.is_dir()]
    if not color_dirs:
        raise ValueError(f"No color subdirectories found in {task_dir}")

    # 收集全部episode用于特征检测
    all_h5 = []
    for color_dir in color_dirs:
        all_h5.extend(sorted(color_dir.rglob("episode_*.hdf5")))
    ...
    for color_dir in color_dirs:
        ...
        for action_dir in [d for d in color_dir.iterdir() if d.is_dir()]:
            action_name = action_dir.name
            hdf5_files = sorted(action_dir.rglob("episode_*.hdf5"))
            if not hdf5_files:
                print(f"  - Skip empty: {color_name}/{action_name}")
                continue

            key = f"{color_name}/{action_name}"
            prompt = (prompt_map or {}).get(key, f"{task_name}/{key}")

            print(f"▶ Subtask {key}: {len(hdf5_files)} episodes, prompt='{prompt}'")
