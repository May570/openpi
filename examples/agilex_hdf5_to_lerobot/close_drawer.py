...
def main():
    # 配置参数
    base_dir = Path("/share/project/section/agilex_6_data/agilex_10_data/open_close")
    repo_id = "tasks"
    ...
    print(f"开始转换close_drawer任务数据集...")
    print(f"基础目录: {base_dir}")
    print(f"数据集ID: {repo_id}")
    
    try:

        task_dir = base_dir / "close_drawer"
        sub_prompt_map = {
            "黑色桌布": "Close the drawer on the black table.",
            "卡其色桌布": "Close the drawer on the khaki table.",
            "深绿桌布": "Close the drawer on the dark green table.",
            "原本颜色": "Close the drawer on the plain table.",
            "枣红桌布": "Close the drawer on the maroon table.",
        }
