...
from scripts.agilex_hdf5_to_lerobot.to_lerobot import port_aloha, DatasetConfig
...
def main():
    ...
    print(f"开始转换pour_drink任务数据集...")
    print(f"基础目录: {base_dir}")
    print(f"数据集ID: {repo_id}")
    
    try:

        task_dir = base_dir / "pour_drink"
        prompt = "Pick up the bottle and pour the drink into a cup."
        ds1 = port_aloha(
            raw_dir=task_dir,
            repo_id=repo_id,
            task=prompt,
