...
def pose6D2quat(pose:np.ndarray):
        ...
        Args:
            pose: 6D姿态数组，shape为(batch_size, 6)
                 前3个元素是第一个向量，后3个元素是第二个向量
            
        Returns:
            欧拉角数组，shape为(batch_size, 3)
        """
        
        column_1 = pose[:, :3]
        column_2 = pose[:, 3:6]

        R = np.stack((column_1, column_2, np.cross(column_1, column_2)), axis=2)
        ...
        for i in range(batch_size):
            # 提取单个3x3旋转矩阵
            R_i = R[i]  # shape: (3, 3)
            euler = rotations.euler_from_matrix(R_i, 0, 1, 2, extrinsic=True)
...
def qpos_2_ee_pose(qpos:np.ndarray):
        ...
        Returns:
            末端执行器姿态数组 [l_ee_trans, l_quat, l_gripper_pos, r_ee_trans, r_quat, r_gripper_pos]
        """
        # 检查输入维度
        if qpos.shape[1] < 89:
            raise ValueError(f"qpos维度不足，需要至少89个元素，当前: {qpos.shape[0]}")
        
        # 提取6D姿态数据
        l_pose6d = qpos[:,83:89]  # 左臂末端执行器姿态 (6D)
        r_pose6d = qpos[:,33:39]  # 右臂末端执行器姿态 (6D)
        
        # 检查姿态数据维度
        if l_pose6d.shape[1] != 6:
            raise ValueError(f"左臂姿态维度错误: {l_pose6d.shape}, 期望: (6,)")
        if r_pose6d.shape[1] != 6:
            raise ValueError(f"右臂姿态维度错误: {r_pose6d.shape}, 期望: (6,)")
        ...
        # 提取位置和夹爪数据
        l_ee_trans = qpos[:,80:83]  # 左臂末端执行器位置 (3D)
        r_ee_trans = qpos[:,30:33]  # 右臂末端执行器位置 (3D)
        
        # 检查位置数据维度
        if l_ee_trans.shape[1] != 3:
            raise ValueError(f"左臂位置维度错误: {l_ee_trans.shape}, 期望: (3,)")
        if r_ee_trans.shape[1] != 3:
            raise ValueError(f"右臂位置维度错误: {r_ee_trans.shape}, 期望: (3,)")
        
        # 拼接所有数据
        result = np.concatenate((r_ee_trans, r_quat, l_ee_trans, l_quat), axis=1)

        expected_dim = 12
        if result.shape[1] != expected_dim:
            raise ValueError(f"输出维度错误: {result.shape[0]}, 期望: {expected_dim}")
...
def load_raw_episode_data(
    ...
    with h5py.File(ep_path, "r") as ep:
        # 转换为float32以匹配LeRobot数据集要求
        state = torch.from_numpy(np.concatenate((qpos_2_joint_positions(ep["/observations/qpos"][:]), qpos_2_ee_pose(ep["/observations/qpos"][:])), axis=1).astype(np.float32))
        action = torch.from_numpy(np.concatenate((qpos_2_joint_positions(ep["/action"][:]), qpos_2_ee_pose(ep["/action"][:])), axis=1).astype(np.float32))
