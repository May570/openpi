...
class Block(nn.Module):
    ...
    def __call__(self, xs, kv_cache, positions, attn_mask, adarms_cond, deterministic=True):  # noqa: FBT002
        ...
        post_attn, kv_cache, attn_weights = attn(pre_attn, positions, attn_mask, kv_cache)
        ...
        return xs, kv_cache, attn_weights
...
class Module(nn.Module):
    ...
    ) -> tuple[Sequence[at.Float[at.Array, "b _t _d"] | None], KVCache, jnp.ndarray]:
        embedded = jax.tree.map(lambda e: e.astype(self.embed_dtype), embedded)
        mask = jnp.asarray(mask)[:, None, :, :]
        if adarms_cond is None:
            adarms_cond = [None] * len(self.configs)

        embedded, kv_cache, attentions = self.layers(embedded, kv_cache, positions, mask, adarms_cond, deterministic)
