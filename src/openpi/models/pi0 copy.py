...
class Pi0(_model.BaseModel):
    ...
    def sample_actions(
        ...
        *,
    ...
    ) -> _tuple[_model.Actions, jnp.ndarray]:
        ...
        _, kv_cache, attentions = self.PaliGemma.llm([prefix_tokens, None], mask=prefix_attn_mask, positions=positions)

        def step(carry):
            x_t, time, attentions = carry
            ...
            (prefix_out, suffix_out), _, attentions = self.PaliGemma.llm(
                [None, suffix_tokens],
                mask=full_attn_mask,
                positions=positions,
                kv_cache=kv_cache,
            ...
            return x_t + dt * v_t, time + dt, attentions
        ...
        x_0, _, attentions= jax.lax.while_loop(cond, step, (noise, 1.0, attentions))
        return x_0, attentions
    ...
    def meanflow_sample_actions(
        ...
        *,
        ...
        t_1d = jnp.ones((batch_size,), dtype=noise.dtype)
        r_1d = jnp.zeros((batch_size,), dtype=noise.dtype)

        suffix_tokens, suffix_mask, suffix_ar_mask, adarms_cond = self.meanflow_embed_suffix(observation, noise, t_1d, r_1d)
        suffix_attn_mask = make_attn_mask(suffix_mask, suffix_ar_mask)
        prefix_attn_mask = einops.repeat(prefix_mask, "b p -> b s p", s=suffix_tokens.shape[1])
        full_attn_mask = jnp.concatenate([prefix_attn_mask, suffix_attn_mask], axis=-1)
        assert full_attn_mask.shape == (
