...
def make_a2d_example() -> dict:
    """Creates a random input example for the A2D policy."""
    return {
        "state": np.ones((16,)),
        "images": {
...
class A2DInputs(transforms.DataTransformFn):
    ...
    # If true, this will convert the joint and gripper values from the standard A2D space to
    # the space used by the pi internal runtime which was used to train the base model.
    adapt_to_pi: bool = True

    # The expected cameras names. All input cameras must be in this set. Missing cameras will be
    # replaced with black images and the corresponding `image_mask` will be set to False.
    EXPECTED_CAMERAS: ClassVar[tuple[str, ...]] = ("cam_left_wrist", "cam_right_wrist", "cam_high")
    ...
    def __call__(self, data: dict) -> dict:
        ...
        # Add the extra images (wrist cameras).
        extra_image_names = {
...
class A2DOutputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        # Only return the first 16 dims (A2D has 16 dimensions).
...
def _gripper_to_angular(value):
    # A2D transforms the gripper positions into a linear space. The following code
    # reverses this transformation to be consistent with pi0 which is pretrained in
    # angular space.
    #
    # These values are coming from the A2D code:
    ...
    # There are 4096 total encoder counts and a2d uses a zero of 2048.
    # Converting this to radians means that the normalized inputs are between (0.5476, 1.6296)
    return _normalize(value, min_val=0.5476, max_val=1.6296)


def _gripper_from_angular(value):
    # Convert from the gripper position used by pi0 to the gripper position that is used by A2D.
...
def _decode_a2d(data: dict, *, adapt_to_pi: bool = False) -> dict:
    # state is [right_arm_joint_angles, right_gripper, left_arm_joint_angles, left_gripper]
    # dim sizes: [7, 1, 7, 1]
...
def _decode_state(state: np.ndarray, *, adapt_to_pi: bool = False) -> np.ndarray:
    if adapt_to_pi:
        ...
        # Reverse the gripper transformation that is being applied by the A2D runtime.
