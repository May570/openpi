...
def run_pour(resume: bool, skip: int) -> int:
    """执行一次 pour.py，返回 returncode"""
    script = Path(__file__).parent / "pour.py"
    if not script.exists():
        print(f"找不到 pour.py: {script}")
        sys.exit(1)
...
def main():
    ...
    while True:
        ...
        if current == 0:
            ...
            code = run_pour(resume=False, skip=0)
        else:
            print(f"尝试 resume（skip={current}）")
            code = run_pour(resume=True, skip=current)
